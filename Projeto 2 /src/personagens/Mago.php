...
class Mago extends Personagem implements Combatente {
    private int $carga = 0;
    private bool $cargaPronta = false; // Nova flag para controlar o bônus
    ...
    // Chamado pela Partida no início do turno
    public function verificarStatusTurno(): ?string {
        if ($this->carga > 0) {
            $this->carga--;
            if ($this->carga === 0) {
                $this->cargaPronta = true; // Ativa o bônus para a próxima ação
    ...
    public function habilidadeEspecial1(Personagem $alvo): string {
        ...
        $dano = ($this->ataque * 1.8) * $multiplicador;
        $alvo->receberDano($dano);
        $this->setEnergia($this->getEnergia() - 35);

        if ($multiplicador > 1.0) {
            return "🔥 BOLA DE FOGO ANIMAL! {$this->nome} usou a carga e causou {$dano} de dano!";
        }
